--[[
	DOORS Script với UI Redzlib - Fixed Version
	Đã sửa lỗi ESP và tất cả bugs
]]
        ...
        pcall(function()
            if v.Name == "KeyObtain" and v:IsA("Model") then
                if not v:FindFirstChild("KeyESP") then
                    local esp = CreateESP(v:FindFirstChildWhichIsA("BasePart"), "🔑 KEY", Color3.new(1, 1, 0))
                    if esp then
                        esp.Name = "KeyESP"
                        esp.Parent = v
                    end
            ...
            for _, v in pairs(room:GetDescendants()) do
                if v.Name == "GoldPile" and v:IsA("Model") then
                    if not v:FindFirstChild("GoldESP") then
                        local esp = CreateESP(v:FindFirstChildWhichIsA("BasePart"), "💰 GOLD", Color3.new(1, 0.84, 0))
                        if esp then
                            esp.Name = "GoldESP"
                            esp.Parent = v
                        end
            ...
            for _, v in pairs(room:GetDescendants()) do
                if v.Name == "Drawer" and v:IsA("Model") then
                    if not v:FindFirstChild("DrawerESP") then
                        local esp = CreateESP(v:FindFirstChildWhichIsA("BasePart"), "📦 Drawer", Color3.new(0.5, 0.5, 0.5))
                        if esp then
                            esp.Name = "DrawerESP"
                            esp.Parent = v
                        end
            ...
            for _, v in pairs(room:GetDescendants()) do
                if (v.Name == "Wardrobe" or v.Name == "Locker") and v:IsA("Model") then
                    if not v:FindFirstChild("ClosetESP") then
                        local esp = CreateESP(v:FindFirstChildWhichIsA("BasePart"), "🚪 " .. v.Name, Color3.new(0.5, 0.25, 0))
                        if esp then
                            esp.Name = "ClosetESP"
                            esp.Parent = v
                        end
            ...
            Connections.EntityESP = Workspace.ChildAdded:Connect(function(child)
                ...
                if Flags.EntityESP then
                    if child.Name == "RushMoving" or child.Name == "AmbushMoving" or child.Name == "Eyes" or child.Name == "FigureRagdoll" then
                        local esp = CreateESP(child:FindFirstChildWhichIsA("BasePart") or child, "⚠️ " .. child.Name:gsub("Moving", ""), Color3.new(1, 0, 0))
